function Git-CleanBranches {
  ...
  param (
    ...
    [Parameter()]
    [string[]]
    $BranchesToFilterOut = @('master', 'main', 'development', 'release'),
  ...
  Git-StashPushApply {
    ...
    # ^(?!.*(master|main|\*)).*$
    # filter out branches contains 'master' or 'main' and the current branch (marked by *)
    ...
    % {
      ...
      if (-not $KeepRemote) {
        ...
        if(0 -eq $LASTEXITCODE) {
          Write-Notice "Successfully deleted remote branch: $branch"
        }
      }
